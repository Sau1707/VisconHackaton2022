    ...
    return (
        <div style={{ height: props.height ? props.height : 270 }} className={style.barBox}>
            <div style={{ marginLeft: props.left ? margin : null, marginRight: props.right ? margin : null }} className={style.bar}>
                ...
                <div style={{ top: 50 }} className={`${style.dot} ${height > steps[0] ? style.dotDone : style.dotMissing}`}>
                    {height > steps[0] ? <Check color="black" size={50} /> : <Question color="black" size={50} onClick = {() => {setHeight(height + 35)}}/>}
                </div>
                <div style={{ top: 225 }} className={`${style.dot} ${height > steps[1] ? style.dotDone : style.dotMissing}`}>
                    {height > steps[1] ? <Check color="black" size={50} /> : <Question color="black" size={50} onClick = {() => {setHeight(height + 35)}}/>}
                </div>
                <div style={{ top: 400 }} className={`${style.dot} ${height > steps[2] ? style.dotDone : style.dotMissing}`}>
                    {height > steps[2] ? <Check color="black" size={50} /> : <Question color="black" size={50} onClick = {() => {setHeight(height + 35)}}/>}
                </div>
